...
for (var i = 1; i <=pageLength; i++) {
  setTimeout( function (i) {
    var mainUrl ="http://www.cromaretail.com/display-product-grid.aspx?strip=strip&test=test&CategoryID=10&MinPrice=1389&MaxPrice=77999&pageNumber=";
    mainUrl = mainUrl +i;
    console.log(mainUrl)
    y('http://www.cromaretail.com/display-product-grid.aspx?strip=strip&test=test&CategoryID=10&MinPrice=1389&MaxPrice=77999&pageNumber=1')
        .select([{
          $root: 'article',
          name: 'article h2',
          price: 'article h3',
        ...
        .format(function(obj) {
          ...
          xray(xlink)
              ...
              .run(function(err,result) {
                sub.push(result);
                fs.writeFile ( "./mobile/inner/mobileinner.json", JSON.stringify (sub, null , '\t' ));
                return result
              })
          obj.link = link;
          thumbDownload.get (obj.image);
          thumbDownload.dest ( './mobile' );
          thumbDownload.run ();
        ...
        .delay(2000)
        .write('./mobile/mobileoutter.json')
