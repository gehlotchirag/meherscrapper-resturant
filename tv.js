...
for (var i = 1; i <=pageLength; i++) {
  setTimeout( function (i) {
    var mainUrl ="http://www.cromaretail.com/display-product-grid.aspx?strip=strip&test=test&CategoryID=34&MinPrice=9988&MaxPrice=5899900&pageNumber=";
    mainUrl = mainUrl +i;
    console.log(mainUrl)
    y(mainUrl)
        .select([{
        ...
        .format(function(obj) {
          ...
          xray(xlink)
              ...
              .run(function(err,result) {
                sub.push(result);
                fs.writeFile ( "./tv/inner/tvinner.json", JSON.stringify (sub, null , '\t' ));
                return result
              })
          obj.link = link;
          thumbDownload.get (obj.image);
          thumbDownload.dest ( './tv' );
          thumbDownload.run ();
